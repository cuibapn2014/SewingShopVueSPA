<template>
  ...
  >
    ...
    <label class="block text-sm my-2">
        ...
        <label class="block text-sm my-2">
          <span class="text-gray-700 dark:text-gray-400">Tên thành phẩm<span class="text-red-500">*</span></span>
      ...
      <AutoComplete
          ...
          :selected="this.data_edit?.ingredient_type?.name"
        ...
        <AutoComplete
          ...
          label="Công đoạn"
          :options="this.opt_stage"
          :selected="this.selectedStage ?? this.data_edit?.stage_product?.name"
          @search="this.searchStage"
...
export default {
  ...
  watch: {
    is_open_modal: async function (n, o) {
      if (!o) {
        this.opt_providers = this.opt_types = this.opt_unit_cals = this.opt_stage = [];
        this.selectedProvider = this.selectedType = this.selectedUnitCal = this.selectedStage = null;
        this.display = [null]
        this.data_edit = {}
      }
  ...
  methods: {
    async getDataById() {
      if (this.id)
      await ingredientService.findById(this.id).then((res) => {
          if (res.data) {
            const data = res.data.data;
            this.data_edit = toRaw(data)
    ...
    async searchIngredientType(data) {
      ...
      await ingredientTypeService.getDataSelect(term).then((res) => {
        if (res.data.code == 200) {
          ...
          this.opt_types = res.data.data.filter(item => item.value != 1);
        }
      });
    },
    ...
    async searchStage(data) {
      const term = {
        name: data,
      };
      await stageService.getDataSelect(term).then((res) => {
        if (res.data.code == 200) {
          this.opt_stage = this.opt_stage.filter(
            (item) => item.value == this.selectedStage
          );
          this.opt_stage = res.data.data;
        }
      });
    },
