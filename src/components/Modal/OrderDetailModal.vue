<template>
  ...
  >
    ...
    >
      ...
      >
        <div
          class="bg-[#ffffff] dark:bg-gray-800 z-10 dark:text-gray-200 lg:w-2/4 md:w-full sm:w-full mx-auto h-auto p-4 shadow-md rounded-lg max-h-full overflow-auto"
          @click.stop=""
        >
          <div class="flex items-center justify-between border-bottom">
            <h2 class="text-xl py-1 text-gray-800 dark:text-gray-200">
              Chi tiết đơn hàng
            </h2>
            <span class="flex items-center text-gray-800 dark:text-gray-200">
              Tạo lúc
              {{
                this.$moment(this.order?.created_at).format("DD/MM/YYYY HH:mm")
              }}</span
            >
            <span class="flex items-center text-gray-800 dark:text-gray-200">
              bởi
              <strong class="ml-1"
                >{{ this.order?.user?.role.name }} -
                {{ this.order?.user?.name }}</strong
              >
              <img
          ...
          <div
            class="w-full border-bottom py-2 flex flex-col text-gray-800 dark:text-gray-200"
          >
            ...
            <span class="w-100 flex">
              ...
              >
                <path
                  ...
                  clip-rule="evenodd"
                /></svg
              >Địa chỉ: {{ this.order?.customer?.address }}</span
            >
            <span class="w-100 flex">
              <svg
                xmlns="http://www.w3.org/2000/svg"
              ...
              >
                <path
                  ...
                  clip-rule="evenodd"
                /></svg
              >Yêu cầu khách hàng: {{ this.order?.note }}</span
            >
          </div>
          <hr />
          <div
          ...
          >
            ...
            <div
              class="grid grid-cols-6 gap-x-1 gap-y-2 my-3 items-center text-gray-800 dark:text-gray-200"
            >
              <!-- <span class="text-center">#</span> -->
              <span class="col-span-3 px-3">Tên sản phẩm - Số lượng</span>
            ...
            <div
              ...
              :class="{
                'bg-gray-200 dark:bg-gray-900 hover:dark:bg-gray-50 hover:dark:text-gray-700':
                  !this.isOddRow(index),
              }"
              v-for="({ id, product, quality, amount, price }, index) in this
                .order?.detail"
              :key="id"
            >
            ...
            ++index
              ...
              <span class="col-span-3 px-3"
                >{{ product.name }} x {{ amount }}
                {{ product.unit_cal.name }}</span
              >
              <span class="text-center">{{ quality && quality.name }}</span>
              <span class="text-center">{{
            ...
            <ul>
              <li class="flex justify-between">
                ...
                <span class="font-bold text-base text-green-500">{{
                  this.formatPrice(
                    this.order?.total / (1 + this.order?.vat / 100)
                  )
                }}</span>
              </li>
              <li class="flex justify-between">
          ...
          >
            <span class="text-base flex items-center">
              ...
              >
                {{ this.$moment(this.order?.finish_date).format("DD/MM/YYYY") }}
                - {{ expireTime > 0 ? `Còn ${expireTime} ngày` : "Đã hết hạn" }}
              </span>
            </span>
            <button
              class="px-3 py-2 bg-indigo-500 hover:bg-indigo-600 duration-150 ease-in text-white rounded-lg mt-2"
              @click="closeModal"
            >
              Đóng
            </button>
...
export default {
  ...
  computed: {
    expireTime() {
      const now = Date.now();
      const expire = this.order?.finish_date && new Date(this.order?.finish_date);
