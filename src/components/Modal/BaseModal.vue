<template>
  ...
  >
    <div
      v-show="isOpenModal"
      class="fixed inset-0 flex items-end transition duration-150 bg-black bg-opacity-50 sm:items-center sm:justify-center z-50"
      id="backdrop-overlay"
      @click="isBackdrop() ? closeModal : null"
      @keydown.escape="isBackdrop() ? closeModal : null"
    >
      <transition
...
export default {
  ...
  methods: {
    ...
    isBackdrop() {
      return this.backdrop !== undefined && this.backdrop
        ? !this.backdrop
        : true;
    },
  },
