    ...
    >
      ...
      >
        ...
        <td class="px-4 py-3 text-sm">
          {{
            item.reviewer_date
              ? new Date(item.reviewer_date).toLocaleDateString("vi-VN")
              : ""
          }}
        </td>
        <td class="px-4 py-3 text-sm">
          <img
            v-tooltip.top-start="`${item.user.name} - ${item.user.role.name}`"
        ...
        <td class="px-4 py-3 text-sm flex items-center">
          ...
          <div v-if="item.status == 1" class="inline-block relative group">
            <ul
              class="absolute hidden text-gray-700 pt-1 right-0 top-[25] group-hover:block z-50"
