<template>
    <div>
        ...
        <BaseTable :field_list="this.fieldList" :data_paginate="dataPaginate" :path_current="'provider'">
            <div v-if="is_load"
            ...
            <tr v-for="(item, idx) in renderData.data?.data" :key="item.id"
                ...
                <td class="px-4 py-3 text-sm">
                    <span :class="{
                        ...
                        'px-2 py-1 font-semibold leading-tight rounded-full dark:text-white bg-red-100 text-red-700 dark:bg-red-600': (item.status == 2),
                    }">
...
export default {
    ...
    data() {
        return {
            fieldList: [
                'Tên', 'Liên hệ', 'Địa chỉ', 'Trạng thái', 'Ghi chú'
            ],
    ...
    methods: {
        ...
        getStatus(status) {
            switch (status) {
                case 1: return "Đang hợp tác";
                case 2: return "Ngưng hợp tác";
            }
        }
    }
}
