<template>
    <BaseTable :field_list="this.fieldList" :data_paginate="dataPaginate" path_current="ingredient">
        <div v-if="is_load"
        ...
        <tr v-for="(item, idx) in renderData.data?.data" :key="item.id"
            ...
            <td class="px-4 py-3">
                ...
                :src="item.images.length > 0 ? `${this.url}/img/${item.images[0].urlImage}` : `${this.url}/img/placeholder.jpg`" alt="" />
            </td>
            <td class="px-4 py-3 text-sm">
                {{ item.Ten }}
            </td>
...
export default {
    ...
    data() {
        return {
            fieldList: [
                'Mã NPL', 'Hình ảnh', 'Tên', 'Cung cấp bởi', 'Giá', 'Địa chỉ', 'Liên hệ', 'Số lượng tồn', 'Tồn thực tế', 'Ghi chú'
            ],
