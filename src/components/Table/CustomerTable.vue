<template>
    <div>
        ...
        <BaseTable :field_list="this.fieldList" :data_paginate="dataPaginate" :path_current="'customer'">
            <div v-if="is_load"
            ...
            <tr v-for="(item, idx) in renderData.data?.data" :key="item.id"
                ...
                <td class="px-4 py-3 text-sm">
                    <img v-tooltip.top-start="'{{ item.user.name }}'"
                ...
                <td class="px-4 py-3 text-sm flex items-center">
                    ...
                    <button v-tooltip="'Xóa'" title="Xóa" @click="openModal(item.id)" :key="item.id"
                        class="flex items-center justify-between px-2 py-2 text-sm font-medium leading-5 text-purple-600 rounded-lg dark:text-gray-400 focus:outline-none focus:shadow-outline-gray"
                        aria-label="Delete">
                        <svg class="w-5 h-5" aria-hidden="true" fill="currentColor" viewBox="0 0 20 20">
                            <path fill-rule="evenodd"
...
export default {
    ...
    data() {
        return {
            fieldList: [
                'Khách hàng', 'Liên hệ', 'Địa chỉ', 'Thêm bởi', 'Ngày cập nhật'
            ],
