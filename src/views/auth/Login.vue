            ...
            <div class="flex flex-col overflow-y-auto md:flex-row">
                ...
                <div class="flex items-center justify-center p-6 sm:p-12 md:w-1/2">
                    ...
                    <div class="w-full">
                        ...
                        <!-- <p v-if="false" class="bg-red-100 text-red-500 text-left text-sm rounded-md p-2 mb-2 shadow-sm">
                        </p> -->
                        <form action="" method="POST" @submit.prevent="login">
                            <label class="block text-sm">
                                <span class="text-sm text-red-500">{{ validate.user }}</span>
                                ...
                                <input
                                    ...
                                    name="email" placeholder="ex@example.com" v-model="credentials.username" />
                            </label>
                            <label class="block mt-4 text-sm">
                                <span class="text-sm text-red-500">{{ validate.password }}</span>
                            ...
                            <div class="py-2 flex items-center">
                                <input class="w-4 h-4 accent-indigo-600 rounded-md checked:bg-indigo-400"
                                    type="checkbox" name="remember" id="chk_remember" v-model="credentials.remember" />
                                <label class="mx-1 text-[#ffffff] lg:text-black dark:text-gray-200 text-sm"
                                    for="chk_remember">Ghi nhớ cho 30 ngày</label>
                            </div>
                            <!-- You should use a button here, as the anchor is only used for the example  -->
                            <button
                                class="block w-full px-4 py-2 mt-4 text-sm font-medium leading-5 text-center text-white transition-colors duration-150 bg-indigo-600 border-transparent rounded-lg active:bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:shadow-outline-indigo">
                        ...
                        <!-- <button
                                class="flex items-center justify-center w-full px-4 py-2 text-sm font-medium leading-5 text-white text-gray-700 transition-colors duration-150border-gray-300 rounded-lg dark:text-gray-400 active:bg-transparent hover:border-gray-500 focus:border-gray-500 active:text-gray-500 focus:outline-none focus:shadow-outline-gray">
                            ...
                            <button
                                class="flex items-center justify-center w-full px-4 py-2 mt-4 text-sm font-medium leading-5 text-white text-gray-700 transition-colors duration-150border-gray-300 rounded-lg dark:text-gray-400 active:bg-transparent hover:border-gray-500 focus:border-gray-500 active:text-gray-500 focus:outline-none focus:shadow-outline-gray">
                                <svg class="w-4 h-4 mr-2" aria-hidden="true" viewBox="0 0 24 24" fill="currentColor">
                                    <path
...
<script>
import loginImage from "../../assets/images/login-office.jpeg"
            ...
            if (!this.isSubmit) {
                ...
                this.$store.dispatch('account/login', this.credentials).then(() => {
                    ...
                    Object.keys(error).forEach(er => {
                        let msg = Array.isArray(error[er]) ? error[er][0] : error[er]
                        toast.error(msg, {
