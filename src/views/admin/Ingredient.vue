...
import ActionIngredientModal from "../../components/Modal/ActionIngredientModal.vue";
...
export default {
  ...
  created() {
    ...
    this.fetchDataIngredient(null, defaultSearchQuery);
  },
  mounted() {
    emitter.on("pagechanged", async (url) => {
      await this.fetchDataIngredient(url);
    });
  },
  beforeUnmount() {
  ...
  methods: {
    async fetchDataIngredient(url = null, query = null) {
      this.isLoadData = true;
      if(url) url = url.includes('?') ? url + '&type=1' : '?type=1'
      else query = query ? query + '&type=1' : 'type=1'
    ...
    searchSubmit() {
      ...
      this.delaySearch = setTimeout(() => {
        ...
        this.fetchDataIngredient(null, serialize);
      }, 250);
    }
  },
};
